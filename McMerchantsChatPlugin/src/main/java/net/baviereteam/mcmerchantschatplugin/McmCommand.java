...
public class McmCommand {
	...
	private final McMerchantsService mcMerchantsClient;
	public McmCommand(McMerchantsChatPlugin plugin, String mcMerchantsUrl) {
		this.plugin = plugin;
		this.mcMerchantsClient = new McMerchantsService(mcMerchantsUrl);
	}

	public static LiteralCommandNode<CommandSourceStack> getMcmCommand(McmCommand commandClass) {
		return Commands.literal("mcm")
				.then(Commands
						.argument("item", ArgumentTypes.resource(RegistryKey.ITEM))
						.executes(commandClass::onMcmCommand)
				)
				.build();
	}
	
	public int onMcmCommand(CommandContext<CommandSourceStack> context) {
